...
tree_post_file=paste("/shares/CIBIO-Storage/CM/scratch/users/aitor.blancomiguez/analyses/nw_metanalysis/strainphlan/output/",sgb,"/RAxML_bestTree.t__",sgb,".StrainPhlAn3.tre.post.png", sep="")
metadata=read.csv(meta_file, sep="\t")
result=anpan_pglmm(meta_file=meta_file, tree_file=tree_file, outcome="non_westernized", covariates=c("country"))
result$loo$comparison
png(tree_post_file, width=1500, height=700)
plot_tree_with_post(tree_file = tree_file, meta_file=meta_file, fit = result$pglmm_fit, covariates=c("country"), outcome="non_westernized", labels="sample_id")
dev.off()
