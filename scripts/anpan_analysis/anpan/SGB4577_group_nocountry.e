...
(1/4) Checking inputs.
Dropping 8934 samples from the metadata (out of 12870 in total) not present in the tree.
...
! Cannot fork when running 'bin/diagnose' (system error -12, Unknown error -12) @unix/processx.c:533 (processx_exec)
---
Backtrace:
 1. anpan::anpan_pglmm(meta_file = meta_file, tree_file = tree_file, outcome = "non_w…
 2. pglmm_fit$cmdstan_diagnose()
 3. self$runset$run_cmdstan_tool("diagnose")
 4. withr::with_path(c(toolchain_PATH_env_var(), tbb_path()), run_log <- wsl_c…
 5. base::force(code)
 6. cmdstanr:::wsl_compatible_run(command = target_exe, args = c(sapply(self$output_files…
 7. base::do.call(processx::run, run_args)
 8. (function (command = NULL, args = character(), error_on_status = TRUE, …
 9. process$new(command, args, echo_cmd = echo_cmd, wd = wd, windows_verbatim_…
10. local initialize(...)
