        ...
        for SGB in SGBs:
            ...
            if (data.loc[data["Lifestyle"]=="non-Westernized", gene].sum()/data.loc[data["Lifestyle"]=="non-Westernized", gene].count() >= 0.1) or \
               (data.loc[data["Lifestyle"]=="Westernized", gene].sum()/data.loc[data["Lifestyle"]=="Westernized", gene].count() >= 0.1):
                ...
                mod = smf.logit("Q(\"%s\") ~ Lifestyle + Contamination + Completeness" %gene, data=data)
...
def main(datatable_file):
    ...
    if not os.path.isfile(datatable_file.replace("db_tables", "oddsratio_metaanalyses_results3x")):
        meta_analysis( measure_differential_genes2(datatable, genes, SGBs), genes, SGBs, datatable_file.replace("db_tables", "oddsratio_metaanalyses_results3") )

    metaa = pd.read_csv(datatable_file.replace("db_tables", "oddsratio_metaanalyses_results3"), sep="\t", header=0, index_col=0, low_memory=False)
    ...
    metaa.to_csv(datatable_file.replace("db_tables", "oddsratio_metaanalyses_results3").replace(".tsv", "_fdr_adjusted.tsv"), sep="\t", header=True, index=True)
